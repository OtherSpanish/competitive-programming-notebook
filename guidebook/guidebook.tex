...
En $[0..n]$ falta un número: suma esperada $-$ suma real.
...
Recorrido lineal. \textbf{Complejidad:} $O(n)$.
\begin{lstlisting}[style=cpp]
...
Par con $|a_i+a_j|$ mínimo: ordenar + dos punteros desde los extremos.
\textbf{Complejidad:} $O(n\log n)$ (por el sort).
\begin{lstlisting}[style=cpp]
sort(a.begin(), a.end());
...
¿Existe par con $a_j-a_i=d$? Ordenar + dos punteros.
...
Máximo (o mínimo) de una función \textbf{unimodal}: parte el rango en
tres. \textbf{Complejidad:} $O(\log n)$. (NO es $O(n\log n)$.)
\begin{lstlisting}[style=cpp]
...
En arreglo \textbf{ordenado}, divide con números de Fibonacci (solo
sumas/restas, sin división). \textbf{Complejidad:} $O(\log n)$.
\begin{lstlisting}[style=cpp]
...
En arreglo \textbf{ordenado}: dobla el índice hasta pasar el objetivo,
luego binaria en ese bloque. Ideal si el rango es enorme/ilimitado.
...
En arreglo \textbf{ordenado}: salta en bloques de $\sqrt{n}$ y luego
barre el bloque. \textbf{Complejidad:} $O(\sqrt{n})$.
\begin{lstlisting}[style=cpp]

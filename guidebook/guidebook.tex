...
\textbf{Idea:} un arreglo de tamaño fijo donde los índices ``dan la
vuelta'' con módulo: al pasar del final se regresa a la posición $0$.
Guardas un \texttt{ini} y un \texttt{tam}; agregar y leer es $O(1)$ sin
mover datos. Cuando se llena, el nuevo pisa al más viejo. Ideal como
cola de tamaño fijo o ventana deslizante.
\textbf{Complejidad:} push/get $O(1)$.
\begin{lstlisting}[style=cpp]
struct VectorCircular {
  vector<int> buf; int cap, ini=0, tam=0;
  VectorCircular(int c):buf(c),cap(c){}
  void push(int x){ buf[(ini+tam)%cap]=x;
...
\textbf{Idea:} mantiene sumas de prefijos permitiendo \emph{actualizar
un elemento} y \emph{consultar la suma de un prefijo}, ambas en
$O(\log n)$ (un arreglo simple solo logra una de las dos rápido). El
truco: cada índice $i$ ``cubre'' un bloque de tamaño $i\,\&\,(-i)$ (su
bit menos significativo), así que subiendo/bajando por esos bloques
tocas solo $\log n$ posiciones. Suma de $[l,r]=$
query$(r)-$query$(l-1)$. \textbf{Complejidad:} update/query
$O(\log n)$.
\begin{lstlisting}[style=cpp]
void update(int i,long long v){       // suma v en pos i
  for(; i<=n; i+=i&(-i)) t[i]+=v; }   // O(log n)
long long query(int i){ long long s=0;
  for(; i>0; i-=i&(-i)) s+=t[i]; return s; } // O(log n)
\end{lstlisting}
\begin{lstlisting}[style=java]
...
Precalcula, para cada posición, el mínimo de los rangos de largo
$1,2,4,\dots$ ($2^k$). Cualquier rango $[l,r]$ se cubre con dos bloques
de largo $2^k$ que se solapan, y como el mínimo es idempotente el
solape no molesta: la consulta son dos lecturas. Sirve igual para
máximo o gcd. \textbf{Complejidad:} construir $O(n\log n)$, consulta
$O(1)$. (Si el arreglo cambia, usa Segment Tree: $O(\log n)$/op.)
\begin{lstlisting}[style=cpp]
...
\textbf{Idea:} usa un \emph{heap} (montículo) para ordenar in-place.
Primero reorganiza el arreglo en un max-heap (el mayor queda en la
raíz) en $O(n)$. Luego, repetidamente: intercambia la raíz (el mayor)
con el último elemento activo —dejándolo ya ordenado al final— y
``hunde'' la nueva raíz para restaurar el heap. Cada extracción cuesta
$O(\log n)$. No usa memoria extra, pero no es estable.
\textbf{Complejidad:} $O(n\log n)$ siempre (mejor, peor y promedio).
\begin{lstlisting}[style=cpp]
...
Igual que quicksort particiona alrededor de un pivote, pero como sabes
en qué lado cae la posición $k$, recurres a \emph{un solo} lado y
descartas el otro. Eso baja el costo promedio de $O(n\log n)$ a $O(n)$
(cada nivel procesa la mitad: $n+n/2+n/4+\dots=2n$). El peor caso
$O(n^2)$ ocurre con pivotes malos (se evita eligiendo el pivote al
azar). \textbf{Complejidad:} promedio $O(n)$, peor $O(n^2)$.
\begin{lstlisting}[style=cpp]
int quickSelect(vector<int>&a,int lo,int hi,int k){
  ...
  if(k<=j) return quickSelect(a,lo,j,k);   // k a la izq
  if(k>=i) return quickSelect(a,i,hi,k);   // k a la der
  return a[k]; }
\end{lstlisting}
\begin{lstlisting}[style=py]
def quick_select(a, lo, hi, k):
    if lo == hi: return a[lo]
    p = a[(lo+hi)//2]; i, j = lo, hi
    while i <= j:
        while a[i] < p: i += 1
        while a[j] > p: j -= 1
        if i <= j:
            a[i], a[j] = a[j], a[i]; i += 1; j -= 1
    if k <= j: return quick_select(a, lo, j, k)
    if k >= i: return quick_select(a, i, hi, k)
    return a[k]
\end{lstlisting}
